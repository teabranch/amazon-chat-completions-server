...
from src.open_bedrock_server.api.errors import http_exception_handler
from src.open_bedrock_server.api.middleware.auth import verify_api_key
...
def test_verify_api_key_missing():
    ...
    assert response.json() == {
        "error": {"message": "Not authenticated", "type": "api_error", "code": 403}
    }
...
def test_verify_api_key_invalid():
    ...
    assert response.status_code == 403
    assert response.json() == {
        "error": {"message": "Invalid API key", "type": "api_error", "code": 403}
    }
