...
from src.amazon_chat_completions_server.adapters.bedrock.bedrock_adapter import BedrockAdapter
...
from src.amazon_chat_completions_server.adapters.bedrock.titan_strategy import TitanStrategy


class TestBedrockAdapterRouting:
    """Test that BedrockAdapter routes to the correct strategies."""

    @patch('src.amazon_chat_completions_server.adapters.bedrock.bedrock_adapter.app_config')
    @patch('src.amazon_chat_completions_server.adapters.bedrock.bedrock_adapter.APIClient')
    def test_strategy_routing(self, mock_api_client, mock_app_config):
        ...
        for model_id, expected_strategy_class in test_cases:
            with patch('src.amazon_chat_completions_server.adapters.bedrock.bedrock_models.get_bedrock_model_id', return_value=model_id):
    ...
    def test_unsupported_model_raises_error(self, mock_api_client, mock_app_config):
        ...
        with patch('src.amazon_chat_completions_server.adapters.bedrock.bedrock_models.get_bedrock_model_id', return_value=unsupported_model_id):
