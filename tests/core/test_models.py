...
def test_chat_completion_chunk():
    ...
    chunk_choice = ChatCompletionChunkChoice(index=0, delta=delta, finish_reason=None)

    chunk = ChatCompletionChunk(
        id="chunk_123", choices=[chunk_choice], model="test-model", created=1234567890
    )
...
def test_model_provider_info():
    model_info = ModelProviderInfo(
        id="model-123", provider="test-provider", display_name="Test Model"
    )
