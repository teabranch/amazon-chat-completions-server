...
def test_chat_command_non_streaming(
    ...
    with patch(
        ...
        mock_response.raise_for_status.return_value = (
            None  # Prevent raise_for_status from erroring
        )
...
def test_chat_command_tool_calls(
    ...
    with patch(
        ...
        mock_response2_data = {
            "choices": [{"message": {"content": "Based on the search results..."}}]
        }
        ...
        def custom_side_effect(*args, **kwargs):
            if "json" in kwargs and "messages" in kwargs["json"]:
                actual_messages_payloads_sent.append(
                    copy.deepcopy(kwargs["json"]["messages"])
                )
            else:
            ...
            if not responses_to_return:
                raise AssertionError(
                    "Mock make_api_request called more times than expected or responses_to_return list exhausted."
                )
        ...
        assert result.exit_code == 0, (
            f"CLI exited with error: {result.output} {result.exception} traceback: {result.exc_info}"
        )
