...
def test_format_message(formatter):
    ...
    assert "[bold green]Assistant[/bold green]" in formatter.format_message(
        "assistant", "Hi there"
    )
    assert "Hi there" in formatter.format_message("assistant", "Hi there")

    # Test system message
    assert "[bold yellow]System[/bold yellow]" in formatter.format_message(
        "system", "System message"
    )
    assert "System message" in formatter.format_message("system", "System message")

    # Test tool message
    assert "[bold magenta]Tool[/bold magenta]" in formatter.format_message(
        "tool", "Tool response"
    )
