...
class BedrockToOpenAIAdapter(BaseLLMAdapter):
    ...
    def __init__(self, openai_model_id: str, **kwargs):
        ...
        logger.info(f"BedrockToOpenAIAdapter initialized for OpenAI model: {self.openai_model_id}")
    ...
    def convert_bedrock_to_openai_request(
        ...
        if isinstance(bedrock_request, BedrockClaudeRequest):
            ...
            raise ValueError(f"Unsupported Bedrock request type: {type(bedrock_request)}")
    
    def _convert_claude_to_openai(self, claude_request: BedrockClaudeRequest) -> ChatCompletionRequest:
        ...
        for bedrock_msg in claude_request.messages:
            openai_content = self._convert_bedrock_content_to_openai(bedrock_msg.content)
            messages.append(Message(
                role=bedrock_msg.role,
    ...
    def _convert_titan_to_openai(self, titan_request: BedrockTitanRequest) -> ChatCompletionRequest:
    ...
    def _convert_bedrock_content_to_openai(self, content: Union[str, List[BedrockContentBlock]]) -> Union[str, List[Dict[str, Any]]]:
    ...
    def _convert_openai_to_claude_response(self, openai_response: ChatCompletionResponse) -> BedrockClaudeResponse:
        ...
        content_blocks = []
        if message.content:
            content_blocks.append(BedrockContentBlock(type="text", text=message.content))
    ...
    def _convert_openai_to_titan_response(self, openai_response: ChatCompletionResponse) -> BedrockTitanResponse:
        ...
        # Create Titan result
        result = {
            "tokenCount": openai_response.usage.completion_tokens if openai_response.usage else 0,
            "outputText": message.content or "",
            "completionReason": completion_reason
        }
        
        return BedrockTitanResponse(
            inputTextTokenCount=openai_response.usage.prompt_tokens if openai_response.usage else 0,
            results=[result]
        )
    ...
    def convert_from_provider_stream_chunk(self, provider_chunk: Any, original_request: ChatCompletionRequest) -> ChatCompletionChunk:
        ...
        return self.openai_adapter.convert_from_provider_stream_chunk(provider_chunk, original_request)
    ...
    async def stream_chat_completion_bedrock(
        ...
        async for openai_chunk in self.openai_adapter.stream_chat_completion(openai_request):
            # Convert OpenAI chunk to Bedrock format
            bedrock_chunk = self._convert_openai_chunk_to_bedrock(openai_chunk, original_format)
            if bedrock_chunk:
                yield bedrock_chunk
