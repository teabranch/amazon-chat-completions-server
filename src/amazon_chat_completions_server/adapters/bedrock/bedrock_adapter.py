...
from ...core.exceptions import ConfigurationError, ModelNotFoundError, LLMIntegrationError, APIRequestError
from ...utils.api_client import APIClient
from ...utils.config_loader import app_config
from .bedrock_models import get_bedrock_model_id # To map generic names to specific Bedrock IDs
...
# Import other strategies as they are created (e.g., LlamaStrategy, CohereStrategy)
...
class BedrockAdapter(BaseLLMAdapter):
    ...
    def __init__(self, model_id: str, **kwargs):
        ...
        logger.info(f"BedrockAdapter initialized for model: {model_id} (resolved to {self.specific_bedrock_model_id}) using strategy: {self.strategy.__class__.__name__}")
    ...
    def _get_strategy(self, bedrock_model_id: str) -> BedrockAdapterStrategy:
        ...
        if bedrock_model_id.startswith("anthropic.claude"):
            ...
            if "embed" in bedrock_model_id:
                 raise ModelNotFoundError(f"Bedrock model {bedrock_model_id} appears to be an embedding model. This adapter is for chat completions. Use an embedding-specific adapter.")
            return TitanStrategy(bedrock_model_id, get_param_func)
        # Add other model families here, e.g.:
        # elif bedrock_model_id.startswith("meta.llama"):
        #     return LlamaStrategy(bedrock_model_id, get_param_func)
        ...
        #     return AI21JurassicStrategy(bedrock_model_id, get_param_func)
        # elif bedrock_model_id.startswith("mistral."):
        #     return MistralStrategy(bedrock_model_id, get_param_func)
        else:
            supported_prefixes = ["anthropic.claude", "amazon.titan"]
            logger.error(f"Unsupported Bedrock model ID: {bedrock_model_id}. Supported prefixes: {supported_prefixes}")
            raise ModelNotFoundError(f"No strategy found for Bedrock model ID: {bedrock_model_id}. Supported model families are Claude, Titan.")
    ...
    def convert_from_provider_response(self, provider_response: Dict[str, Any], original_request: ChatCompletionRequest) -> ChatCompletionResponse:
    ...
    def convert_from_provider_stream_chunk(self, provider_chunk: Dict[str, Any], original_request: ChatCompletionRequest, response_id: str, created_timestamp: int) -> ChatCompletionChunk:
        """Delegates to the strategy to handle a Bedrock stream chunk."""
        return self.strategy.handle_stream_chunk(provider_chunk, original_request, response_id, created_timestamp)

    async def chat_completion(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        if request.stream:
            raise APIRequestError("BedrockAdapter.chat_completion called with stream=True. Use stream_chat_completion for streaming.")
    ...
    async def stream_chat_completion(
        ...
        if not request.stream:
            logger.warning("BedrockAdapter.stream_chat_completion called with stream=False. Forcing stream=True.")
            request.stream = True
        ...
        stream_response_id = f"br-{self.strategy.__class__.__name__.lower()}-{uuid.uuid4()}"
        ...
        async for provider_chunk_data in provider_stream: # provider_chunk_data is a dict from bedrock stream
            ...
            elif converted_chunk and converted_chunk.choices is not None and len(converted_chunk.choices) == 0 and converted_chunk.id: # Empty chunk to signal stream start etc.
