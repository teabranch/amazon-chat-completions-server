...
from ...core.exceptions import APIRequestError, LLMIntegrationError, UnsupportedFeatureError
...
class TitanStrategy(BedrockAdapterStrategy):
    ...
    def _format_messages_to_titan_input_text(self, messages: List[Message], system_prompt: Optional[str]) -> str:
        ...
        for msg in messages:
            ...
            if msg.role == "assistant":
                ...
                logger.warning(f"'tool' role for Titan model ({self.model_id}) will be formatted as plain text. Ensure content is a string.")
            ...
            if isinstance(msg.content, list):
                ...
                logger.warning(f"Titan model {self.model_id} received list content for role {msg.role}. Joining text parts. Multimodal content not directly supported.")
    ...
    def prepare_request_payload(self, request: ChatCompletionRequest, adapter_config_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        if request.tools or request.tool_choice:
            raise UnsupportedFeatureError("Amazon Titan models do not support OpenAI-style 'tools' or 'tool_choice' parameters directly through this adapter strategy.")

        system_prompt, processed_messages = self._extract_system_prompt_and_messages(request.messages)
        input_text = self._format_messages_to_titan_input_text(processed_messages, system_prompt)

        text_generation_config = {}
        max_tokens = request.max_tokens if request.max_tokens is not None \
            else self._get_default_param("max_tokens", default_value=app_config.DEFAULT_MAX_TOKENS_TITAN)
        if max_tokens is not None:
            text_generation_config["maxTokenCount"] = max_tokens
        else:
            raise APIRequestError("maxTokenCount (max_tokens) is required for Titan models.")

        temperature = request.temperature if request.temperature is not None \
            else self._get_default_param("temperature", default_value=app_config.DEFAULT_TEMPERATURE_TITAN)
        ...
        for generic_param, titan_param in titan_specific_params.items():
            ...
            elif generic_param in adapter_config_kwargs and adapter_config_kwargs[generic_param] is not None:
    ...
    def parse_response(self, provider_response: Dict[str, Any], original_request: ChatCompletionRequest) -> ChatCompletionResponse:
        # Titan response structure: {"inputTextTokenCount": N, "results": [{"tokenCount": M, "outputText": "...", "completionReason": "..."}]}
        if not provider_response.get("results") or not isinstance(provider_response["results"], list) or len(provider_response["results"]) == 0:
    ...
    async def handle_stream_chunk(self, chunk_data: Dict[str, Any], original_request: ChatCompletionRequest, response_id: str, created_timestamp: int) -> ChatCompletionChunk:
        ...
        if delta_content is None and finish_reason is None:
             logger.debug(f"Skipping Titan stream chunk with no content or finish reason: {chunk_data}")
             # Return an empty chunk to satisfy type, it might be filtered by caller
             return ChatCompletionChunk(id=response_id, choices=[], created=created_timestamp, model=original_request.model, object="chat.completion.chunk")
        ...
        if delta_content is None and not finish_reason:
            delta_content = "" # Should not happen if filtered above, but as a safeguard

        choice_delta = ChoiceDelta(content=delta_content, role="assistant" if delta_content is not None else None)
