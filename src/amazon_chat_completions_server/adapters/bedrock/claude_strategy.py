...
class ClaudeStrategy(BedrockAdapterStrategy):
    ...
    def __init__(self, model_id: str, get_default_param_func: callable):
        ...
        logger.info(f"ClaudeStrategy initialized for model: {self.model_id} with anthropic_version: {self.anthropic_version}")
    ...
    def prepare_request_payload(self, request: ChatCompletionRequest, adapter_config_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_tokens = request.max_tokens if request.max_tokens is not None \
            else self._get_default_param("max_tokens", default_value=app_config.DEFAULT_MAX_TOKENS_CLAUDE)
        ...
        temperature = request.temperature if request.temperature is not None \
            else self._get_default_param("temperature", default_value=app_config.DEFAULT_TEMPERATURE_CLAUDE)
    ...
    def parse_response(self, provider_response: Dict[str, Any], original_request: ChatCompletionRequest) -> ChatCompletionResponse:
        ...
        stop_reason = self._map_finish_reason(provider_response.get("stop_reason", "unknown"))
    ...
    async def handle_stream_chunk(self, chunk_data: Dict[str, Any], original_request: ChatCompletionRequest, response_id: str, created_timestamp: int) -> ChatCompletionChunk:
        ...
        if chunk_type == "content_block_delta":
            ...
            if delta_info.get("type") == "text" or delta_info.get("type") == "text_delta":  # Handle both text and text_delta types
