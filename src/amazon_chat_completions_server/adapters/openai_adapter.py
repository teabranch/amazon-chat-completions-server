...
from ..core.exceptions import APIRequestError, LLMIntegrationError, ConfigurationError, UnsupportedFeatureError
from ..utils.api_client import APIClient # Assuming APIClient is synchronous for now, or make this adapter async
...
class OpenAIAdapter(BaseLLMAdapter):
    ...
    def convert_to_provider_request(self, request: ChatCompletionRequest) -> Dict[str, Any]:
        ...
        for msg in request.messages:
            ...
            if msg.tool_calls:
                provider_msg["tool_calls"] = msg.tool_calls # OpenAI expects this directly
        ...
        temperature = request.temperature if request.temperature is not None else self._get_default_param("temperature", default_value=app_config.DEFAULT_TEMPERATURE_OPENAI)
        if temperature is not None:
            payload["temperature"] = temperature

        max_tokens = request.max_tokens if request.max_tokens is not None else self._get_default_param("max_tokens", default_value=app_config.DEFAULT_MAX_TOKENS_OPENAI)
        ...
        supported_openai_params = ["top_p", "presence_penalty", "frequency_penalty", "stop", "logit_bias", "user"]
        for param_name in supported_openai_params:
            if param_name in self.config_kwargs and self.config_kwargs[param_name] is not None:
                ...
                if hasattr(request, param_name) and getattr(request, param_name) is not None:
                    payload[param_name] = getattr(request, param_name)
                else:
                    payload[param_name] = self.config_kwargs[param_name]
            elif hasattr(request, param_name) and getattr(request, param_name) is not None:
                payload[param_name] = getattr(request, param_name)
    ...
    async def chat_completion(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        ...
        if request.stream:
            ...
            raise APIRequestError("chat_completion called with stream=True. Use stream_chat_completion for streaming.")
    ...
    async def stream_chat_completion(
        ...
        if not request.stream:
            ...
            logger.warning("stream_chat_completion called with stream=False. Forcing stream=True.")
        ...
        async for provider_chunk in stream_response:
            ...
            if choices:
                ...
                if choice.get('delta') or choice.get('finish_reason'):
                    yield self.convert_from_provider_stream_chunk(provider_chunk, request)
