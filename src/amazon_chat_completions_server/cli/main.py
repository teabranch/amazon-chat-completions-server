...
CONFIGURABLE_KEYS = [
    ...
    {"name": "AWS_ACCESS_KEY_ID", "prompt": "AWS Access Key ID (leave blank if using AWS_PROFILE or IAM role)", "sensitive": False},
    {"name": "AWS_SECRET_ACCESS_KEY", "prompt": "AWS Secret Access Key (leave blank if using AWS_PROFILE or IAM role)", "sensitive": True},
    {"name": "AWS_PROFILE", "prompt": "AWS Profile Name (e.g., default, my-profile; used if AWS keys above are blank)", "sensitive": False},
...
def chat(model: str, server_url: str, api_key: str, stream: bool, session: str, session_name: str):
    """Start an interactive chat session."""
    asyncio.run(_async_chat(model, server_url, api_key, stream, session, session_name))

async def _async_chat(model: str, server_url: str, api_key: str, stream: bool, session: str, session_name: str):
    ...
    if session:
        try:
            ...
            console.print(f"Continuing chat session: {chat_session.name or chat_session.id}", style="green")
        except ValueError as e:
            console.print(f"[bold red]Error:[/bold red] {str(e)}")
            return
    else:
    ...
    async def stream_response(websocket_url: str, payload: dict, headers: dict):
        try:
            async with websockets.connect(
                f"{websocket_url}/v1/chat/completions/stream",
            ...
            console.print(f"\n[bold red]WebSocket Error:[/bold red] {str(e)}")
            return None
    ...
    while True:
        try:
            ...
            if stream:
                ...
                response_content = await stream_response(websocket_url, payload, headers)
                if response_content:
                    chat_session.messages.append({"role": "assistant", "content": response_content})
            else:
                try:
                    ...
                    if "tool_calls" in assistant_message:
                        formatter.print_message("assistant", "I need to use some tools to help you.")
                        tool_responses = []
                        for tool_call in assistant_message["tool_calls"]:
                            ...
                            tool_response = Prompt.ask("[b magenta]Tool Response[/b magenta]")
                    ...
                    chat_session.messages.append({"role": "assistant", "content": assistant_content})
