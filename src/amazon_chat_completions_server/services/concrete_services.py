...
from ..adapters.base_adapter import BaseLLMAdapter # OpenAIAdapter and BedrockAdapter inherit from this
...
class ConcreteLLMService(AbstractLLMService):
    ...
    def __init__(self, adapter: BaseLLMAdapter):
        if not isinstance(adapter, BaseLLMAdapter):
            raise LLMIntegrationError(f"Adapter must be an instance of BaseLLMAdapter, got {type(adapter)}")
        self.adapter = adapter
        self.provider_name = adapter.__class__.__name__.replace("Adapter", "").lower()
        logger.info(f"ConcreteLLMService initialized with {adapter.__class__.__name__} for model {adapter.model_id}")

    async def chat_completion(
        self,
        messages: List[Message],
        model_id: Optional[str] = None, # If provided, can override adapter's default, though adapter is typically model-specific
        stream: bool = False,
        ...
        try:
            if stream:
                if not hasattr(self.adapter, 'stream_chat_completion'):
                    raise NotImplementedError(f"Adapter {self.adapter.__class__.__name__} does not support streaming.")
                ...
                if not hasattr(self.adapter, 'chat_completion'):
                    raise NotImplementedError(f"Adapter {self.adapter.__class__.__name__} does not support non-streaming chat_completion.")
                response: ChatCompletionResponse = await self.adapter.chat_completion(request_dto)
                return response
            ...
            raise LLMIntegrationError(f"Unexpected error in {self.provider_name} service: {e}") from e
...
class OpenAIService(ConcreteLLMService):
    def __init__(self, adapter: BaseLLMAdapter):
        ...
        if not type(adapter).__name__ == 'OpenAIAdapter': # Basic check
             raise LLMIntegrationError("OpenAIService must be initialized with an OpenAIAdapter.")
...
class BedrockService(ConcreteLLMService):
    def __init__(self, adapter: BaseLLMAdapter):
        ...
        if not type(adapter).__name__ == 'BedrockAdapter': # Basic check
             raise LLMIntegrationError("BedrockService must be initialized with a BedrockAdapter.")
