...
from src.amazon_chat_completions_server.core.exceptions import ServiceAuthenticationError, ServiceModelNotFoundError, ServiceApiError, ServiceUnavailableError
import logging

logger = logging.getLogger(__name__)

class OpenAIService(AbstractLLMService):
    def __init__(self, api_key: Optional[str] = None, **kwargs): # Added **kwargs to match factory
    ...
    async def _map_to_core_response(self, openai_response) -> ChatCompletionResponse:
        core_choices = [
            CoreChatCompletionChoice(
                index=choice.index,
                message=Message(role=choice.message.role, content=choice.message.content, tool_calls=choice.message.tool_calls),
    ...
    async def _handle_non_streaming(
        ...
        try:
            ...
            raise ServiceAuthenticationError(f"OpenAI authentication failed: {e.message}")
        except NotFoundError as e:
            logger.error(f"OpenAI Model Not Found Error: {e} (Model: {model_id})")
            raise ServiceModelNotFoundError(f"OpenAI model not found or you do not have access: {model_id}. Detail: {e.message}")
        except APIError as e:
            logger.error(f"OpenAI API Error: {e} (Model: {model_id}, Status: {e.status_code}, Type: {type(e).__name__})")
            if e.status_code == 503:
                raise ServiceUnavailableError(f"OpenAI service unavailable: {e.message}")
            raise ServiceApiError(f"OpenAI API error: {e.message} (Status: {e.status_code}, Type: {type(e).__name__}, Param: {e.param}, Code: {e.code})")
        except Exception as e:
            logger.error(f"Unexpected error during OpenAI call: {type(e).__name__} - {e} (Model: {model_id})")
            raise ServiceApiError(f"Unexpected error communicating with OpenAI: {type(e).__name__} - {str(e)}")
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        logger.info(f"OpenAIService: Sending chat completion. Model: {model_id}, Stream: {stream}")
    ...
    async def list_models(self) -> List[ModelProviderInfo]:
        ...
        try:
            ...
            raise ServiceAuthenticationError(f"OpenAI authentication failed while listing models: {e.message}")
        except APIError as e:
            logger.error(f"OpenAI API Error listing models: {e}")
            raise ServiceApiError(f"OpenAI API error listing models: {e.message} (Status: {e.status_code})")
        except Exception as e:
