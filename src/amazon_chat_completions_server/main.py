...
from .utils import logger_setup, config_loader 
...
async def run_openai_example():
    ...
    try:
        ...
        openai_service = LLMServiceFactory.get_service("openai", model_id="gpt-4o-mini") # Chat model
        ...
        logger.error(f"An unexpected error occurred during OpenAI example: {e}", exc_info=True)

async def run_bedrock_claude_example():
    logger.info("\n--- Running Bedrock Claude Example ---")
    if not (config_loader.app_config.AWS_ACCESS_KEY_ID and config_loader.app_config.AWS_SECRET_ACCESS_KEY):
    ...
    try:
        ...
        messages = [
            ...
            Message(role="user", content="Compose a short poem about asynchronous programming.")
        ]
        ...
        if response.choices and response.choices[0].message:
            ...
            logger.error("Bedrock Claude Non-Stream response had no choices or message.")
        ...
        logger.error(f"An unexpected error occurred during Bedrock Claude example: {e}", exc_info=True)
...
async def run_bedrock_titan_example():
    ...
    try:
        ...
        messages = [
            ...
            Message(role="user", content="When was the first programmable computer invented?")
        ]
        ...
        logger.error(f"An unexpected error occurred during Bedrock Titan example: {e}", exc_info=True)
