...
class RequestFormatDetector:
    ...
    def is_openai_format(request_data: Dict[str, Any]) -> bool:
        ...
        has_messages = "messages" in request_data and isinstance(request_data["messages"], list)
    ...
    def is_bedrock_claude_format(request_data: Dict[str, Any]) -> bool:
        ...
        if has_max_tokens and has_messages and (has_claude_fields or has_claude_tools or has_claude_tool_choice):
            return True
    ...
    def is_bedrock_titan_format(request_data: Dict[str, Any]) -> bool:
        ...
        if has_text_gen_config:
            ...
            if isinstance(config, dict):
                ...
                titan_config_fields = {"maxTokenCount", "temperature", "topP", "stopSequences"}
                if "maxTokenCount" in config or any(field in config for field in titan_config_fields):
                    valid_config = True
        
        return has_input_text and valid_config
    
    @staticmethod
    def get_format_confidence(request_data: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        if any(param in request_data for param in ["temperature", "max_tokens", "top_p"]):
