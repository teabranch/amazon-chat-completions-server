...
import json
import asyncio
from typing import Any, Dict, AsyncGenerator, Callable, Optional, Union
...
def get_openai_client():
    ...
    if not app_config.OPENAI_API_KEY:
        logger.warning("OpenAI API key is not configured. OpenAI client will not be initialized.")
        return None
...
class APIClient:
    ...
    def __init__(self):
        ...
        self.bedrock_runtime_client: Optional[Any] = None # boto3 client, type hinted as Any for simplicity
    ...
    def get_openai_client(self) -> openai.AsyncOpenAI:
        ...
        if self.openai_client is None:
            self.openai_client = openai.AsyncOpenAI(api_key=self.app_config.OPENAI_API_KEY)
    ...
    def get_bedrock_runtime_client(self) -> Any:
        ...
        if self.bedrock_runtime_client is None:
            ...
            if not (static_keys_present or profile_name_present):
            ...
            try:
                self.bedrock_runtime_client = session.client(
                    ...
                    config=botocore.config.Config(
                        retries={'max_attempts': 0, 'mode': 'standard'} # We use tenacity for retries at a higher level
                    )
                )
                logger.info(f"Bedrock runtime client initialized successfully for region: {session.region_name or 'Default'}.")
    ...
    async def make_openai_chat_completion_request(
        ...
        stream: bool = False
    ) -> Union[openai.types.chat.ChatCompletion, AsyncGenerator[openai.types.chat.ChatCompletionChunk, None]]:
        """Makes a request to OpenAI Chat Completions API."""
        client = self.get_openai_client()
        
        try:
            logger.debug(f"OpenAI Request: model={request_payload.get('model')}, stream={stream}, messages[:1]={request_payload.get('messages', [])[:1]}")
            ...
            logger.error(f"OpenAI API Status Error (status: {e.status_code}): {e.message}")
            if 500 <= e.status_code < 600:
                raise APIServerError(f"OpenAI Server Error (status: {e.status_code}): {e.message}") from e
            else:
                raise LLMIntegrationError(f"OpenAI API Status Error (status: {e.status_code}): {e.message}") from e
        except Exception as e:
    ...
    async def make_bedrock_request(
        ...
        try:
            ...
            if not client.meta.region_name: # Should have been set or warned during client init
                 raise ConfigurationError("AWS region not configured or discoverable for Bedrock client.")
        except ConfigurationError as e: # Catch error from get_bedrock_runtime_client specifically
        ...
        try:
            logger.debug(f"Bedrock Request: model_id={model_id}, stream={stream}, body_keys={body.keys()}")
            if stream:
                ...
                logger.debug(f"Bedrock invoke_model_with_response_stream called for {model_id}.")
            ...
            logger.error(f"Bedrock API ClientError: Code={error_code}, Message={error_message}, Status={status_code}")
            
            if error_code == 'AccessDeniedException':
                raise AuthenticationError(f"Bedrock Access Denied: {error_message}") from e
            if error_code == 'ThrottlingException':
                raise RateLimitError(f"Bedrock Throttled: {error_message}") from e
            if error_code == 'ValidationException':
                raise APIRequestError(f"Bedrock Validation Error: {error_message}") from e
            if error_code == 'ResourceNotFoundException':
                 raise APIRequestError(f"Bedrock Resource Not Found (e.g. model ID): {error_message}") from e
            if status_code and 500 <= status_code < 600:
                raise APIServerError(f"Bedrock Server Error (status: {status_code}): {error_message}") from e
    ...
    async def _handle_bedrock_stream(self, event_stream: Any) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            for event in event_stream:
                ...
                if chunk:
                    ...
                    logger.debug(f"Bedrock stream chunk received: {list(chunk_data.keys()) if isinstance(chunk_data, dict) else 'Non-dict chunk'}")
                    yield chunk_data
                elif 'internalServerException' in event or 'modelStreamErrorException' in event or 'throttlingException' in event:
                    error_details = event.get('internalServerException') or event.get('modelStreamErrorException') or event.get('throttlingException')
                    ...
                    if 'throttlingException' in event:
                        raise RateLimitError(f"Bedrock stream throttled: {error_details.get('message')}")
                    else:
                        raise APIServerError(f"Error in Bedrock stream: {error_details.get('message')}")
        except Exception as e:
...
async def _test_openai():
    ...
    try:
        ...
        async for chunk in await api_client.make_openai_chat_completion_request(payload, stream=True):
...
async def _test_bedrock_claude():
    ...
    claude_model_id = "anthropic.claude-3-sonnet-20240229-v1:0" # Or another claude model
    claude_payload = {
    ...
    try:
        ...
        async for chunk in await api_client.make_bedrock_request(claude_model_id, claude_payload, stream=True):
            ...
            elif chunk.get('type') == 'message_stop': # For Claude 3 Haiku, Sonnet, Opus
                print(f"\nStream finished. Stop Reason: {chunk.get('amazon-bedrock-invocationMetrics',{}).get('outputTokenCount')} tokens") # Metrics are in a separate chunk
        print()
