...
class Message(BaseModel):
    ...
    content: Optional[Union[str, List[Dict[str, Any]]]] = None  # content can be string or list of content blocks for multimodal, and can be None for tool calls
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.role != "tool" and self.content is None and self.tool_calls is None:
            raise ValueError("Either content or tool_calls must be provided for non-tool messages")
        ...
        if self.tool_calls:
            for tool_call in self.tool_calls:
                ...
                if "id" not in tool_call or "type" not in tool_call or "function" not in tool_call:
                    raise ValueError("Tool call missing required fields: id, type, function")
                ...
                if "name" not in tool_call["function"] or "arguments" not in tool_call["function"]:
                    raise ValueError("Tool call function missing required fields: name, arguments")
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.tools:
            for tool in self.tools:
                ...
                if "name" not in tool["function"] or "description" not in tool["function"] or "parameters" not in tool["function"]:
                    raise ValueError("Tool function missing required fields: name, description, parameters")
...
class ChatCompletionChunkChoice(BaseModel):
    delta: ChoiceDelta
    finish_reason: Optional[Literal["stop", "length", "tool_calls", "content_filter", "end_turn", "max_tokens", "stop_sequence"]] = None
    index: int
...
class BedrockClaudeRequestBody(BaseModel):
    ...
    max_tokens: int = Field(..., alias="max_tokens_to_sample") # Example of alias for older versions if needed
