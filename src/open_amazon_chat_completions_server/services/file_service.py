...
class FileService:
    ...
    def __init__(
        ...
        self.AWS_ROLE_SESSION_NAME = AWS_ROLE_SESSION_NAME or app_config.AWS_ROLE_SESSION_NAME
        self.AWS_WEB_IDENTITY_TOKEN_FILE = AWS_WEB_IDENTITY_TOKEN_FILE or app_config.AWS_WEB_IDENTITY_TOKEN_FILE
        ...
        try:
            ...
            if self.validate_credentials:
                ...
                try:
                    ...
                    logger.error(f"AWS credentials not found or incomplete for FileService: {e}")
                    ...
                    if error_code in ["InvalidClientTokenId", "SignatureDoesNotMatch"]:
                        logger.error(f"AWS STS authentication failure for FileService: {e}")
                        raise ConfigurationError(f"AWS authentication failed (STS) for FileService: {e}")
                    logger.warning(f"AWS STS GetCallerIdentity warning for FileService: {e}")
            else:
                logger.info("Skipping AWS credential validation for FileService (validate_credentials=False)")
            ...
            logger.error(f"AWS credentials issue during FileService S3 client initialization: {e}")
            raise ConfigurationError(f"AWS credentials not found or incomplete for FileService S3: {e}")
        except BotoCoreError as e:
            logger.error(f"BotoCoreError during FileService S3 client initialization: {e}")
            raise ConfigurationError(f"AWS SDK (BotoCore) error during FileService init: {e}")
        except Exception as e:
            logger.error(f"Failed to initialize FileService S3 client: {e}")
            raise ConfigurationError(f"Failed to initialize AWS S3 client for FileService: {e}")
    ...
    def _create_aws_session(self) -> boto3.Session:
        ...
        static_keys_present = bool(app_config.AWS_ACCESS_KEY_ID and app_config.AWS_SECRET_ACCESS_KEY)
        ...
        logger.info(f"AWS authentication methods detected for FileService - Static keys: {static_keys_present}, "
        ...
        if static_keys_present:
            logger.info("Creating boto3 session with static AWS credentials for FileService.")
            ...
            logger.info(f"Creating boto3 session with AWS SSO profile for FileService: {self.AWS_PROFILE}")
            return boto3.Session(
                profile_name=self.AWS_PROFILE, 
                region_name=self.AWS_REGION
            )
        elif profile_present:
            logger.info(f"Creating boto3 session with profile for FileService: {self.AWS_PROFILE}")
            return boto3.Session(
                profile_name=self.AWS_PROFILE, 
                region_name=self.AWS_REGION
            )
        elif role_arn_present:
            logger.info(f"Creating boto3 session with role assumption for FileService: {self.AWS_ROLE_ARN}")
            return self._create_assume_role_session()
        elif web_identity_present:
            logger.info(f"Creating boto3 session with web identity token for FileService: {self.AWS_WEB_IDENTITY_TOKEN_FILE}")
            return self._create_web_identity_session()
        else:
            logger.info(f"Creating boto3 session with default credentials for FileService and region: {self.AWS_REGION}")
    ...
    def _create_assume_role_session(self) -> boto3.Session:
        ...
        try:
            ...
            try:
                ...
                logger.info("Base credentials validated for FileService role assumption.")
            except (NoCredentialsError, PartialCredentialsError) as e:
                logger.error(f"No base credentials available for FileService role assumption: {e}")
                raise ConfigurationError(
            ...
            logger.info(f"Successfully assumed role for FileService: {self.AWS_ROLE_ARN}")
    ...
    def _create_web_identity_session(self) -> boto3.Session:
        ...
        try:
            ...
            if not self.AWS_WEB_IDENTITY_TOKEN_FILE:
                raise ConfigurationError("AWS_WEB_IDENTITY_TOKEN_FILE is required for FileService web identity authentication")
            
            if not os.path.exists(self.AWS_WEB_IDENTITY_TOKEN_FILE):
                raise ConfigurationError(f"FileService web identity token file not found: {self.AWS_WEB_IDENTITY_TOKEN_FILE}")
            ...
            try:
                ...
                sts_client = session.client("sts")
                caller_identity = sts_client.get_caller_identity()
                logger.info(f"Successfully initialized FileService session with web identity token. Account: {caller_identity.get('Account')}")
                return session
            finally:
            ...
            raise ConfigurationError(f"FileService web identity authentication error: {e}")
    ...
    async def upload_file(
        ...
        """
        if not self.s3_bucket:
            raise ConfigurationError("S3_FILES_BUCKET is not configured. Cannot upload files.")
        ...
        try:
            # Upload to S3
            logger.info(f"Uploading file {filename} to S3 bucket {self.s3_bucket} with key {s3_key}")
            ...
            logger.error(f"S3 upload failed for file {filename}: {error_code} - {error_message}")
            
            if error_code == "NoSuchBucket":
                raise ServiceApiError(f"S3 bucket '{self.s3_bucket}' does not exist")
            elif error_code == "AccessDenied":
                raise ServiceApiError(f"Access denied to S3 bucket '{self.s3_bucket}'. Check IAM permissions.")
            else:
